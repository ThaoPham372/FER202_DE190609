...
function ExpensesDashboard() {
  ...
  useEffect(() => {
    if (user?.id) {
      dispatch(fetchExpensesThunk(user.id));
    }
  }, [dispatch, user?.id]);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await dispatch(
          updateExpenseThunk({ id: editingId, updatedExpense: expenseData }),
        );
      } else {
        await dispatch(addExpenseThunk(expenseData));
      }
      handleReset();
    } catch (error) {
  ...
  const handleConfirmDelete = async () => {
    try {
      await dispatch(deleteExpenseThunk(deleteId));
      setShowModal(false);
      setDeleteId(null);
